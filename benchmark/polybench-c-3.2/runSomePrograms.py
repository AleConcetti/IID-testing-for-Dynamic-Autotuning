...
nameOfProgramsToRun = ['covariance', '2mm', 'durbin','gemm', 'symm','syrk']

nameOfCommandToRun=[]
for programToRun in nameOfProgramsToRun:
    for i in range(len(nameList)):
        program=nameList[i]
        if programToRun==program:
            nameOfCommandToRun.append(commandList[i])

end_preprocessing = time.time()
#----------------------PROCESSING----------------------
ex_times_list = processing(NUM_OF_EXECUTIONS,nameOfCommandToRun,nameOfProgramsToRun)
